...
def telnet_update(Host,admin,ip_addr):  # ip_addr is for getting the own ip address, admin is the user_name password ,initial_msg to check with the message with the previous one
	...
	while True:
	    try:
		with open ("/home/pi/Desktop/PC_ER/ER/Energy_Router/main_codes/comm/Network/telnet.txt","r") as f: #open file with f only other wise it qill not work properly
        ...
        for p in msg:
		  ...
		  if '\n' in ip_entry:
		      ...
		      ip_entry =ip[0]
               ...
               mac_entry = Mac(mac =a.strip() ,port_no =int(b.strip()),ip_address=ip_entry.strip())
               if int(b.strip()) in port:
                           if int(b.strip()) not in dup_port:
                               dup_port.append(int(b.strip()))
               port.append(int(b.strip()))
	...
	while True:
		try:
		    with open("/home/pi/Desktop/PC_ER/ER/Energy_Router/main_codes/comm/Network/Net_Table.txt","w") as f:
			fcntl.flock(f,fcntl.LOCK_EX) ## lock  the file
